...
from custom_class import Histogram, Contour
...
class CM3(Scene):
    def construct(self):
        ...
        np.random.seed(7)   # 固定随机数种子，防止每次渲染动画时结果不同
        self.manim_ga2d()

    def manim_ga2d(manim_self):
        # 二维GA动画
        class GA_2D:
            def __init__(
                    self,
                ...
                self.max_fit_x_rec: list[float, float] = []
                self.calculate()
            ...
            def calculate(self):
                ...
                for idx_generation in range(self.num_generations):  # 迭代
                    ...
                    self.max_fit_x_rec.append(self.gray_decoding(elite))
            ...
            def manim_animate(self):
                ...
                histogram_obj = Histogram(
                    data=fitness, axis_range=fit_range, axis_step=fit_step, bin_num=50, size=(4, 2.5),
                    font_size=font_size,
                ...
                max_fit_rec_axes.shift(histogram_obj.obj_axes.c2p(0, 0, 0) - max_fit_rec_axes.c2p(0, 0, 0)).shift(
                    1.5 * RIGHT)
                ...
                max_fit_rec_y_label = MathTex("y", font_size=font_size).move_to(
                    max_fit_rec_y_axes.n2p(max_fit_rec_y_axes.x_max)).shift(RIGHT * 0.3)
                max_fit_rec_obj = VGroup(max_fit_rec_x_axes, max_fit_rec_y_axes, max_fit_rec_x_label,
                                         max_fit_rec_y_label)
                ...
                histogram_all_obj = VGroup(histogram_obj, max_fit_rec_obj, zero_point).center().to_edge(DOWN,
                                                                                                        buff=0.5)  # 包含直方图和历代数据

                # 当代个体
                fig_obj.update_now_point(
                    point_ls=[self.gray_decoding(gene) for gene in self.population_ls],
                    color=YELLOW_COLOR
                )
                fig_obj.update_path_point(path_point=self.max_fit_x_rec, color=BLACK, stroke_color=WHITE, stroke_width=0.2)

                # 标签信息
                # 目标函数
                aim_func_tex_obj = MathTex(
                    r"f(x_{1},x_{2})=20\exp{\left ( -0.2\sqrt{0.5x_{1}^{2}+0.5x_{2}^{2}}\right )} +"
                    r"\exp{\left ( 0.5\cos{2\pi x_{1}}+0.5\cos{2\pi x_{2}}\right )}", font_size=font_size)
                aim_func_tex_obj.set_color(WHITE)

                boundary_tex_obj = MathTex(r"x_{1} \in [-30, 30], x_{2} \in [-30, 30]", font_size=font_size)
                boundary_tex_obj.set_color(WHITE)

                st_tex_obj = MathTex(r"x_{1}+x_{2}<5", font_size=font_size)
                st_tex_obj.set_color(WHITE)
                ...
                info = VGroup(aim_func_tex_obj, boundary_tex_obj, st_tex_obj, rate_info, now_info).arrange(DOWN, buff=0.3)
                info.to_edge(UL, buff=0.5)

                manim_self.add(histogram_all_obj, info)
                manim_self.wait(frame_time)
                manim_self.remove(histogram_all_obj, info)

        x_range = ((-20, 20), (-20, 20))
        aim_func = lambda x1, x2: 20 * np.exp(-0.2 * np.sqrt(x1 ** 2 / 2 + x2 ** 2 / 2)) + \
                                  np.exp(np.cos(0.5 * np.pi * x1) / 2 + np.cos(0.5 * np.pi * x2) / 2)
        colors = [BLUE_E, BLUE_A, PURPLE_A, PURPLE_E]
        fig_obj = Contour(aim_func, x_range=[-20, 20, 10], y_range=[-20, 20, 10],
                          x_length=3.5, y_length=3.5, z_range=[0, 25, 5],
                          axis_config={"include_tip": False, "include_ticks": False},
                          sep_num=100, colors=colors)
        fig_obj.to_edge(UR, buff=0.5).shift(0.8*LEFT)
        fig_obj.add_band([(20, -15), (20, 20), (-15, 20)], fill_color=RED, fill_opacity=0.7, stroke_color=WHITE,
                         stroke_width=1)

        manim_self.add(fig_obj)
        ga_proj = GA_2D(crossing_rate=0.2, mutation_rate=0.08, population_size=20, num_generations=100,objective_func=aim_func, objective_range=x_range)
